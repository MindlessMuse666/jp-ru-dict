...
| ID | Название | Описание | Приоритет |
| --- | --- | --- | --- |
| **NFR-015** | Хэширование паролей | Пароли должны храниться в хэшированном виде с использованием bcrypt | MUST HAVE |
| **NFR-016** | JWT аутентификация | API должно использовать JWT токены для аутентификации запросов | MUST HAVE |
| **NFR-017** | CORS политика | API должно иметь настроенную CORS политику для безопасности | MUST HAVE |
...
| Метод | Эндпоинт | Описание |
| --- | --- | --- |
| **GET** | `/api/words` | получить список слов с пагинацией и поиском |
| **GET** | `/api/words/{id}` | получить слово по ID |
| **POST** | `/api/words` | создать новое слово |
| **PATCH** | `/api/words/{id}` | частично обновить слово по ID |
...
## Требования к аутентификации (AUTH)

| ID | Название | Описание | Приоритет |
| --- | --- | --- | --- |
| **AUTH-001** | Регистрация пользователя | Система должна позволять новым пользователям создавать учетную запись | MUST HAVE |
| **AUTH-002** | Вход в систему | Система должна предоставлять возможность аутентификации зарегистрированных пользователей | MUST HAVE |
| **AUTH-003** | Изоляция данных | Данные каждого пользователя должны быть полностью изолированы от данных других пользователей | MUST HAVE |
| **AUTH-004** | Профиль пользователя | Система должна хранить базовую информацию о пользователе (email, имя) | MUST HAVE |
...
| **name** | VARCHAR(100) | NULLABLE | Имя пользователя |
...
| --- | --- | --- | --- |
| **id** | SERIAL | PRIMARY KEY | Уникальный идентификатор |
| **user_id** | INTEGER | NOT NULL, FOREIGN KEY REFERENCES users(id) | Владелец слова |
